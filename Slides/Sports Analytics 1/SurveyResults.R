...
sportswatched =  na.omit(gather(data=DATA4,5:37,key="Watch",value="Sport")$Sport)
...
sportswatched4 = ifelse(sportswatched3==unique(sportswatched)[4],"Auto/Bike Racing",sportswatched3)
sportswatched5 = ifelse(sportswatched4==unique(sportswatched)[53],"Yard Games",sportswatched4)
sportswatched6 = ifelse(sportswatched5==unique(sportswatched)[11],"Board Games",sportswatched5)
sportswatched7 = ifelse(sportswatched6==unique(sportswatched)[6],"Air Sports",sportswatched6)

unique(sportswatched7)

png('Watched.png',width=1200,height=800)
par(mar=c(14,3,3,1),cex=1.5,cex.main=2.5)
barplot(table(fct_reorder(as.factor(sportswatched7),sportswatched7,.fun=table)),las=2,ylim=c(0,100),main="Sports Watched")
dev.off()

sportsplayed =  na.omit(gather(data=DATA4,38:64,key="Play",value="Sport")$Sport)

unique(sportsplayed)

sportsplayed2 = ifelse(sportsplayed==unique(sportsplayed)[2],"Arcade",sportsplayed)
sportsplayed3 = ifelse(sportsplayed2==unique(sportsplayed)[39],"Competitive Board Games",sportsplayed2)
sportsplayed4 = ifelse(sportsplayed3==unique(sportsplayed)[25],"Combat Sports",sportsplayed3)
